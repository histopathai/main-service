...
echo "Cleaning up orphaned IAM bindings from Terraform state (env: $ENVIRONMENT)..."

# Only clean up dev-prefixed bindings when deploying to dev
if [[ "$ENVIRONMENT" == "dev" ]]; then
  ...
  echo "  - Skipping dev-prefixed cleanup (not in dev environment)"
fi
...
if ! terraform state show google_cloud_run_v2_service.main_service >/dev/null 2>&1; then
  ...
  terraform import \
    ...
    "projects/${TF_VAR_project_id}/locations/${TF_VAR_region}/services/main-service-${ENVIRONMENT}" \
