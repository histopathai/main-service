...
type ImageResponse struct {
	ID            string             `json:"id"`
	Parent        *ParentRefResponse `json:"parent"` // PatientID yerine artık ParentRef var
...
func NewImageResponse(img *model.Image) *ImageResponse {
	return &ImageResponse{
		ID:            img.ID,
		Parent:        NewParentRefResponse(img.Parent), // Common dosyasındaki helper'ı kullanıyoruz
